...
export const getLinkedProductionOrders = createAsyncThunk(
  PRODUCTION_ORDER_GET_LINKED,
  async (
    { salesOrderId, salesOrderMaterialId }: GetLinkedProductionOrdersParams,
    { rejectWithValue },
  ) => {
    try {
      const response = await ApiService.get<ProductionOrder[]>(
        PRODUCTION_ORDER_GET_LINKED + `/${salesOrderId}/${salesOrderMaterialId}/`,
      );
      const { data } = response;
      return data;
    } catch (err: any) {
