...
describe('production order types thunk', () => {
  it('should return production order types on success', async () => {
    const getSpy = vi
      .spyOn(axiosInstance, 'get')
      .mockResolvedValueOnce({ data: mockedSettingsData });

    const dispatch = getDispatch();
    const response = await dispatch(getAllProductionOrderTypes());
...
describe('delete production order type', async () => {
  it('should delete production order type on success', async () => {
    const deleteSpy = vi
      .spyOn(axiosInstance, 'delete')
      .mockResolvedValueOnce({ data: mockedSettingsItem.id });

    const dispatch = getDispatch();
    const response = await dispatch(deleteProductionOrderTypes(mockedSettingsItem.id));
...
describe('post-put production order type', async () => {
  it('should put production order type when given id', async () => {
    const putSpy = vi
    ...
    const dispatch = getDispatch();
    const response = await dispatch(upsertProductionOrderTypes(mockedSettingsItem));
  ...
  it('should post production order type when no id', async () => {
    const postSpy = vi
    ...
    const dispatch = getDispatch();
    const response = await dispatch(upsertProductionOrderTypes(mockedSettingsItemForPost));
